...
class HighSchoolAgreedManager(models.Manager):
    ...
    def get_queryset(self):
        ...
        try:
            w_convention = general_settings.objects.get(setting="ACTIVATE_HIGH_SCHOOL_WITH_AGREEMENT")
            hs_w_convention = w_convention.parameters.get("value", True)
        except general_settings.DoesNotExist:
            hs_w_convention = True

        try:
            wo_convention = general_settings.objects.get(setting="ACTIVATE_HIGH_SCHOOL_WITHOUT_AGREEMENT")
